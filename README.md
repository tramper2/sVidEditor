...
2. **[해상도 및 FPS 설정]** 상단 헤더 영역에서 최종 결과물로 내보낼 비디오 규격 (FHD 1080p, HD 720p, 세로형 9:16, SD 360p 및 60/30/24 fps)을 선택합니다.
3. 좌측 드롭존을 통해 편집할 영상/음원을 가져와 타임라인(Track 1~4)에 적절히 드래그 배치합니다.
4. 영상 클립을 클릭하고 우측 속성 제어판에서 **'로컬 전체 경로'**가 내 컴퓨터 내 실제 절대 경로(예: `D:\Study\WebPage\sVidEditor\source\video1.mp4`)로 지정되었는지 확인합니다.
5. 자막 입력, 비디오 회전(Transpose), 필터 드래그(흑백, 세피아), 줌, 프리뷰 플레이어 직접 드래그 이동, PIP 배치 등을 수행합니다.
