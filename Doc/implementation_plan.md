...
* 프론트엔드 핵심 제어 로직:
  * **상태 관리(State Management)**: 임포트된 미디어 리소스 정보, 타임라인 배치 클립, 재생 시간, 줌 수준 및 **사용자 정의 출력 해상도(가로/세로), 출력 FPS** 관리.
...
3. 상단 설정 메뉴에서 출력 해상도를 `1920x1080`으로 변경하고, 프레임을 `30`으로 변경해 봅니다.
4. 타임라인 지시부의 라벨 텍스트가 `1920x1080 30fps`로 업데이트되는지 확인합니다.
5. 타임라인 비디오 트랙 1에 클립들을 배치하고 마우스 드래그로 순서가 정상적으로 스왑되는지 확인합니다.
6. 자막이나 PIP 영상, 이미지 스티커를 마우스로 잡고 프리뷰 캔버스 내에서 드래그하여 위치를 옮겨봅니다.
7. 프로젝트 파라미터 저장(`project.json`) 및 로컬 렌더링용 Windows 배치 파일(`render.bat`) 내보내기를 수행합니다.
8. 내보낸 배치 파일 내부 및 터미널 프리뷰 창의 FFmpeg 명령어 상에서, 규격화 필터 크기(`scale=1920:1080`, `pad=1920:1080`), 프레임 레이트 옵션(`fps=30`, `-r 30`) 등이 사용자가 설정한 값으로 정상 치환되어 출력되는지 검증합니다.
