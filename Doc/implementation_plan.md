...
* 싱글 페이지 애플리케이션(SPA)의 HTML 구조:
  * **상단 메뉴바**: 
    * 프로젝트 컨트롤 (새 프로젝트, 프로젝트 불러오기, 프로젝트 저장, FFmpeg 스크립트/배치 파일 내보내기, 배포 안내).
    * **출력 렌더링 설정**: 최종 동영상의 해상도(1080p, 720p, vertical 1080x1920, 360p) 및 FPS(60, 30, 24 fps)를 지정하는 드롭다운 셀렉터 추가.
  ...
  * **하단 타임라인**: 
    ...
    * 타임라인 재생 헤드, 시간 표시 눈금자(선택한 해상도 및 FPS 정보 상시 동기화), 줌인/아웃 슬라이더.
...
* 헤더에 추가된 해상도/FPS 드롭다운 셀렉터용 모던 다크 스타일 가이드셋 추가.

### 3. [app.js](file:///D:/Study/WebPage/sVidEditor/app.js)
* 프론트엔드 핵심 제어 로직:
  * **상태 관리(State Management)**: 임포트된 미디어 리소스 정보, 타임라인 배치 클립, 재생 시간, 줌 수준, 출력 해상도, 출력 FPS 관리.
  * **미디어 임포트 강건화**: HTML5 미지원 비디오/오디오 코덱(예: MKV, AVI, WMV, FLAC, WMA 등) 임포트 시 브라우저 메타데이터 로드 실패(`onerror`)가 발생하더라도 강제로 에셋을 등록하여 타임라인 편집 및 FFmpeg 스크립트 생성이 가능하게 설계(임시 지속시간 10초 및 `isPreviewDisabled` 플래그 부여).
  * **확장자 기반 포맷 구분**: 브라우저가 MIME 타입을 빈 값으로 던져주더라도 파일 확장자(`.mp3`, `.wav`, `.m4a`, `.png` 등)를 검사하여 정확한 미디어 종류를 판별하도록 보완.
  * **대체 프리뷰 화면**: 캔버스 렌더러가 `isPreviewDisabled` 미디어를 그릴 때 검은 배경에 '브라우저 프리뷰 불가 포맷 (로컬 FFmpeg 렌더링 지원)' 경고를 띄워 가독성을 개선.
  * **설정 동기화**: 출력 해상도 및 FPS 드롭다운 변경 시 타임라인 FPS 라벨과 전역 상태 객체를 업데이트하고 FFmpeg 명령어를 즉시 재건합니다.
  ...
  * **타임라인 눈금자 그리기 및 스크러빙 (drawRuler & scrub)**: 타임라인 상단의 시간 눈금자 Canvas에 줌 레벨과 가로 스크롤 오프셋을 반영하여 눈금 시작점을 트랙과 일치하게 맞추고, 눈금자를 드래그하여 재생 헤드를 이동시키는 `scrub()` 및 `drawRuler()` 복구 완료.
  * **프로젝트 입출력**: 프로젝트를 저장하거나 불러올 때 현재 지정된 출력 해상도(가로/세로)와 FPS 설정을 직렬화하여 JSON에 완벽히 보존합니다.

### 4. [ffmpeg-generator.js](file:///D:/Study/WebPage/sVidEditor/ffmpeg-generator.js)
* 타임라인 설정을 Windows FFmpeg 명령어로 변환하는 핵심 컴파일러:
  * **동적 출력 규격 표준화**: 사용자가 지정한 가로/세로 해상도 및 FPS에 맞추어 개별 입력 비디오에 대한 결합용 규격화 필터(`scale=outW:outH`, `pad=outW:outH`, `fps=outFPS`)를 유동적으로 적용합니다.
  * 비디오 효과 처리 (회전, 흑백, 세피아, 줌인/아웃, 리버스). 크롭 및 줌아웃 패딩 옵션에도 동적 해상도를 연계합니다.
  * **PIP 오버레이**: 메인 비디오 트랙 위에 서브 트랙 비디오의 스케일 크기 및 드래그 설정된 화면 상 좌표를 `overlay=x=X:y=Y:enable='between(t,S,E)'` 방식으로 합성.
  * **자막 텍스트**: Windows 시스템 폰트를 사용하는 `drawtext` 필터 추가. 사용자가 입력한 자막 텍스트, 실시간 지정한 시간대 및 화면 상 드래그한 좌표 값을 `drawtext=text='TEXT':x=X:y=Y:enable='between(t,S,E)'`로 변형하여 배치합니다. 동적 가로 해상도에 맞춰 정중앙 배치 감지식을 최적화합니다.
...
2. 브라우저가 직접 지원하지 않는 대용량 비디오 코덱(예: `.mkv`, `.avi` 등)을 드래그 앤 드롭으로 가져옵니다.
3. 드롭존이 파일을 거부하지 않고 미디어 라이브러리 목록에 강제로 등록되는지 확인합니다.
4. 비디오 트랙 1에 영상을 배치하고 스왑해보며 순서 교체가 잘 되는지 테스트합니다.
5. 프리뷰 모니터에 `브라우저 프리뷰 불가 포맷 (로컬 FFmpeg 렌더링 지원됨)`이라는 붉은색 경고 박스가 캔버스 영역 내에 이쁘게 노출되는지 확인합니다.
6. 프로젝트 파라미터 저장(`project.json`) 및 로컬 렌더링용 Windows 배치 파일(`render.bat`) 내보내기를 수행합니다.
7. 내보낸 배치 파일 내부에서 파일 전체 로컬 절대 경로(`D:\...\video.mkv`)가 정상 생성되었으며 인코더 프레임 레이트 및 해상도가 정상 매핑되었는지 최종 확인합니다.
8. 로컬 `ffmpeg`를 사용해 `render.bat`을 실행하고 렌더링된 최종 영상이 브라우저 프리뷰와 완벽히 일치하는지 최종 확인합니다.
