...
로컬 렌더링 방식의 웹 동영상 편집기 **sVidEditor**의 모든 핵심 기능 및 추가 수정 작업(타임라인 드래그 오류 수정, 자석식 밀착 정렬 & 스왑 알고리즘 최적화, 자막/오버레이 시간 길이 트림/조절, 프리뷰 모니터 직접 드래그, 사용자 정의 출력 해상도 및 FPS 설정 UI 추가 및 FFmpeg 컴파일러 연동, **브라우저 재생 불가 파일 포맷 강제 임포트 및 확장자 감지 예외 처리 완료**, **drawRuler 정의부 복구 및 가로 스크롤 연동 리렌더링 버그 수정**, **비디오 트랙 1번 자석 효과 비활성화 및 재생헤드 다이아몬드 드래그 연동 복구**, **오버레이 트랙의 자막 추가 버튼을 툴바로 배치하여 조작 사용성 개선**, **선택한 클립에 대한 Delete 키 삭제 단축키 매핑**, **앱 로딩 시 기본 렌더링 설정을 FHD(1920x1080) 30fps로 지정**, **비디오 오디오 음소거 해제 및 오디오 트랙 잡음(지지직) 드리프트 싱크 오류 보완 완료**, FFmpeg 실시간 명령어 매핑 검증, 윈도우 인코딩 오류 차단)을 완료하고 배포했습니다.
...
### ⑦ 비디오 트랙 1번(Video Track 1) 자석 효과 비활성화 (자유 시간 이동 보장)
*   비디오 트랙 1번에 존재하던 자석 정렬 로직(`alignVideo1Clips`) 호출을 마우스 업, 삭제, 그리고 인풋 포커스 아웃 시점에서 전면 비활성화(주석 처리)했습니다. 이를 통해 비디오 트랙 1번에 배치된 클립들도 다른 트랙들과 마찬가지로 시간 간격 제약 없이 타임라인 상의 원하는 곳에 자유롭게 배치하고 이동할 수 있도록 조치했습니다.

### ⑧ 재생헤드(Playhead) 다이아몬드 핸들 직접 드래그 이동 연동
*   눈금자 캔버스 영역 드래그와는 별개로, 재생헤드 최상단에 마킹된 다이아몬드 모양의 붉은색/시안색 손잡이(`.playhead-handle`) 자체를 마우스로 잡고 끄는(Drag & Scrub) 이벤트 처리기를 구현했습니다. 이를 통해 재생헤드 핸들을 직접 드래그해서 프레임을 신속하게 스크러빙하고 정밀 자르기 편집점 탐색을 수행할 수 있게 개선했습니다.

### ⑨ 오버레이 트랙 자막 추가 버튼 툴바 이전 배치
*   오버레이 트랙 레인 내부에 둥둥 떠서 클립 드래그 및 영역 조절 시 마우스 클릭을 겹쳐 방해하던 '자막 추가' 버튼을 타임라인 컨트롤 툴바(`.timeline-toolbar`)의 좌측 영역(분할하기 버튼 옆)으로 깔끔하게 이전했습니다. 이로 인해 오버레이 클립 편집 시 원치 않는 버튼 오클릭이나 간섭 문제를 완벽히 해결했습니다.

### ⑩ 클립 선택 후 Delete(Del) 단축키를 통한 즉각 삭제 구현
*   타임라인 상의 클립을 마우스로 좌클릭하여 선택한 뒤, 키보드의 `Delete` 키를 입력하면 마우스 우측 패널의 삭제 버튼을 직접 누르지 않더라도 즉시 타임라인에서 영구 삭제 처리하는 이벤트를 매핑했습니다. 텍스트 입력박스(자막 내용 기입, 로컬 경로 기입 등)를 포커스하여 작업 중일 때는 텍스트를 삭제할 수 있도록 단축키를 예외 우회 처리(activeElement 검증)했습니다.

### ⑪ 기본 출력 렌더링 해상도 및 FPS 설정 변경
*   웹 에디터를 새로 실행하거나 신규 프로젝트로 리셋할 때 적용되는 기본 출력 설정의 기본값을 기존 HD(1280x720 60fps)에서 영상 업계 표준 FHD(1920x1080 30fps)로 업그레이드했습니다. 이를 위해 HTML 셀렉터 태그의 `selected` 값 및 메인 스크립트 상태 객체(`STATE`)의 가로/세로/FPS 초기 기본 주입 파라미터를 동기화 갱신했습니다.

### ⑫ 비디오 오디오 활성화 및 프리뷰 오디오 잡음(지지직) 제거
*   **비디오 엘리먼트 음소거 해제 & hidden-players 컨테이너 스타일 적용**: 프리뷰 로드 시 비디오 엘리먼트가 캔버스 드로잉용 음소거(`muted=true`)로 생성되는 문제와 더불어, 숨겨진 미디어 엘리먼트들이 `display: none` 내부에서 렌더링될 때 브라우저 보안 및 디코더 최적화 정책으로 인해 오디오 출력이 아예 막히던 현상을 해결했습니다. 이를 위해 컨테이너를 `display: none` 대신 보이지 않는 절대 좌표 및 미세 크기(`.hidden-players` 클래스)로 설정하여 레이아웃을 살려 브라우저 오디오 출력이 활성화되도록 정정하고, 재생 루프 도중 `updateActivePlayersVolumes()`를 통해 `muted=false`를 실시간 부여하도록 했습니다.
*   **브라우저 자동재생 제한(Autoplay Policy) 우회용 웜업(Warm-up) 구현**: 브라우저 보안 정책상 비동기 재생 루프 내부에서 동적으로 재생되는 미디어의 사운드가 차단되는 문제를 해결하기 위해, 사용자가 최초 재생 버튼을 클릭하여 인터랙션이 발생하는 시점에 모든 미디어를 무음 상태로 살짝 재생 후 일시정지시키는 **웜업 언락 기법**(`unlockAllPlayers`)을 구현하여 원본 영상의 오디오 출력을 정상적으로 뚫어냈습니다.
*   **재생 속도(playbackRate) 동적 미세조정(PLL 기법)을 통한 무음 싱크**: 매 프레임 실행되는 싱크 보정 루틴이 미세 오차로 인해 `player.currentTime`을 강제로 빈번히 변경(Seek)함으로써 오디오 버퍼가 깨져 지지직거리는 파열음(Crackling Noise)을 유발하거나, 쿨다운 시스템으로 인해 1초 주기로 소리가 튀던 문제를 종식시켰습니다. 재생 중 오차가 0.08초~1.5초 이내일 때는 `player.playbackRate`를 `0.96배`~`1.04배`로 미세하게 높이거나 낮추어 속도를 통해 소리 끊김(Seek) 없이 부드럽게 싱크를 조율하도록 하고, 1.5초 이상 벌어질 때만 최후의 수단으로 쿨다운 기반 Seek을 하도록 개선했습니다.
*   **비디오 프레임 렌더러(drawVideoClip) 싱크 조건 분리**: 캔버스를 재생 및 스크러빙하는 `drawVideoClip` 함수에서 재생 중(`STATE.isPlaying`)일 때는 무분별한 `currentTime` 보정을 중단하고(싱크 전담 루틴인 `updateActivePlayersStates`에 위임), 일시정지 상태(스크러빙 중)일 때만 기민하게 프레임 탐색을 하도록 분리하여 오디오 지터와 하드웨어 부하를 동시에 최적화했습니다.

### ⑬ 비디오 효과(흑백 등) 렌더링 우회 및 편의성 클릭 연동
*   **Offscreen Buffer Canvas를 통한 GPU 렌더 버그 우회**: 일부 Chromium 계열 브라우저에서 `<video>` 엘리먼트를 Canvas에 복사할 때 하드웨어 가속 최적화(Fast Path)가 개입하여 `ctx.filter` 속성이 완전히 무시되는 현상을 해결했습니다. 효과가 하나라도 적용된 클립의 경우, 비디오의 원본 해상도와 동기화된 임시 버퍼 캔버스(`filterBufferCanvas`)에 텍스트처 프레임을 선 드로잉한 뒤 이를 메인 캔버스에 그리는 **버퍼 렌더 우회 기법**을 구축하여 프리뷰 화면에 흑백(grayscale) 및 세피아(sepia) 효과가 100% 실시간 렌더링되도록 구현했습니다.
*   **효과 클릭 적용(Toggle) 및 UI 편의성 개선**: 기존의 드래그 앤 드롭 방식 외에도, 타임라인에서 특정 클립을 선택한 상태에서 효과 항목(예: '흑백')을 **단순 클릭**하는 것만으로 효과를 즉시 추가하거나 제거할 수 있는 편리한 **토글(Toggle) 적용 메커니즘**을 도입했습니다.
*   **우측 속성 패널 '적용된 효과 목록' 및 개별 삭제 UI 탑재**: 선택한 비디오 클립에 현재 입혀진 효과 목록을 우측 속성 패널에서 직접 한눈에 감시하고, 효과 배지 우측의 삭제 버튼(X)을 눌러 해당 효과만 간편하게 떼어낼 수 있는 삭제 핸들러를 추가 연동했습니다.
